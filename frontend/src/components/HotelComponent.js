...
const ReviewComponent = ({ date, service_score, cleanliness_score, value_score, location_score, sleep_quality_score, rooms_score, review}) => {
    return (
        <div>
            <div>
                {date}
...
export default function HotelComponent({ hotelName }) {
    ...
    return (
      <Modal
        open={true}
        // onClose={handleClose}
        style={{ height: 400, display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', overflow: 'hidden'}}
        >
        <Box
            p={3}
        ...
        <div>
            {hotelInfo.map((review, index) => (
                <ReviewComponent key={index} date={review.date} service_score={review.service_score} cleanliness_score={review.cleanliness_score} value_score={review.value_score} location_score={review.location_score} sleep_quality_score={review.sleep_quality_score} rooms_score={review.rooms_score} review={review.review}></ReviewComponent>
            )
            )}
        </div>
        </Box>
